...
export default function Setup() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/organizations/setup`, {
        method: 'POST',
        headers: {
  ...
  const getGovernmentWarning = () => {
    if (formData.industry === 'Government' && formData.location && formData.location !== 'United States') {
      return (
                  ...
                  >
                    {showIndustryDropdown && (
                      <div
                       ...
                       className="absolute z-10 w-full mt-1 bg-neutral-800 border border-neutral-700 rounded-lg shadow-lg">
                        {industryOptions
                          .filter(option => 
                            option.toLowerCase().includes(formData.industry.toLowerCase())
                          )
                          .map(option => (
                            <div
                              key={option}
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    {securityConcernOptions.map(concern => (
                      <Checkbox
                        ...
                        onChange={() => handleCheckboxChange('securityConcerns', concern)}
                      />
                    ))}
                  </div>
                </div>
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    {regulatoryOptions.map(reg => (
                      <Checkbox
                        key={reg}
                        label={reg}
                        checked={formData.regulatoryRequirements.includes(reg)}
            ...
            <div className="flex flex-col items-center ">
              <div className="bg-neutral-800/50 backdrop-blur-sm p-6 rounded-xl shadow-lg border border-neutral-700 w-80 sticky top-4 hidden lg:block">
                ...
                <p className="text-neutral-300 text-sm mb-4">
                  Stuck at any point? Our support team is here to help you get started.
                </p>
                
                <div className="space-y-4">
                  <div>
                    <div className="text-xs text-neutral-400 mb-1">Call us at</div>
                    <a 
                      href="tel:+2677383510" 
                  ...
                  <div>
                    <div className="text-xs text-neutral-400 mb-1">Email us at</div>
                    <a 
              ...
              <div className="mt-4 bg-neutral-800/50 backdrop-blur-sm p-6 rounded-xl shadow-lg border border-neutral-700 w-80 sticky top-4 hidden lg:block">
                <h3 className="text-lg font-semibold text-white mb-3">Are you a MSP?</h3>
               
                
                <p className="text-neutral-300 text-sm mb-4">
                    Copy and paste this code into your Vyvern account to bind this organization.
                </p>
              ...
              {formData.industry === 'Government' && (
                <div className="mt-4 bg-neutral-800/50 backdrop-blur-sm p-6 rounded-xl shadow-lg border border-neutral-700 w-80 sticky top-4 hidden lg:block">
                  <h3 className="text-lg font-semibold text-white mb-3">Government Information</h3>
                 
                  <p className="text-neutral-300 text-sm mb-4">
                    A representative will be in contact with you shortly to complete the setup.
                  </p>
